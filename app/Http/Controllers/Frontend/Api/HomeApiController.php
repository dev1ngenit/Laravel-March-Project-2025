...
class HomeApiController extends Controller
{
    ...
    public function checkoutStore(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'order_items.*.product_name'   => 'nullable|string',
            'order_items.*.product_color'  => 'nullable|string',
            'order_items.*.product_image'  => 'nullable|string',
            'order_items.*.product_sku'    => 'nullable|string',
        ...
        try {
            ...
            $order = Order::create([
                ...
                'payment_status'               => 'unpaid',
                'status'                       => 'pending',
                'shipped_to_different_address' => $request->has('ship_address') ? 'yes' : 'no',
                'billing_first_name'           => $request->input('billing_first_name'),
                'billing_last_name'            => $request->input('billing_last_name'),
                'billing_email'                => $request->input('billing_email'),
                'billing_phone'                => $request->input('billing_phone'),
